...
function openCourse(code, updateUrl = true) {
  ...
  elements.courseDetail.querySelector(".detail-intro").insertAdjacentHTML("beforebegin", `<div class="detail-actions"><button class="detail-action favorite-button${state.favorites.has(code) ? " active" : ""}" type="button" data-favorite-course="${escapeHtml(code)}">${state.favorites.has(code) ? "★ 관심 과목 저장됨" : "☆ 관심 과목 저장"}</button><button class="detail-action mobile-share-action" type="button" data-share-course="${escapeHtml(code)}"><span class="action-icon kakao-icon" aria-hidden="true"><svg viewBox="0 0 24 24"><path d="M12 4C6.9 4 3 7.1 3 11c0 2.5 1.6 4.7 4.1 5.9L6.2 20l3.7-2.1c.7.1 1.4.2 2.1.2 5.1 0 9-3.1 9-7.1S17.1 4 12 4Z"/></svg></span><span>카카오톡 등으로 공유</span></button><button class="detail-action copy-link-action" type="button" data-copy-course="${escapeHtml(code)}"><span class="action-icon" aria-hidden="true"><svg viewBox="0 0 24 24"><path d="M10.5 13.5a4 4 0 0 0 5.7 0l2.3-2.3a4 4 0 0 0-5.7-5.7l-1.3 1.3M13.5 10.5a4 4 0 0 0-5.7 0l-2.3 2.3a4 4 0 0 0 5.7 5.7l1.3-1.3"/></svg></span><span>URL 복사</span></button><span class="share-status" aria-live="polite"></span></div>`);
